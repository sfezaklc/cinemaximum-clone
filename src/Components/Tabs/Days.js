...
const Days = ({selectedCinemas, id}) => {
    ...
    const HandleBuyTicket = (element,e) => {
        ...
        navigate(`/buyticket/${id}/${selectedCinemas[0].location}/${e.target.innerText}`,{state: {date: element.date, month: element.month}})
    }
        ...
        >
            {days.map((element, index) => (
                <Tab eventKey={element.day} key={index} title={`${element.date} ${element.month} ${element.day}`} className="text-start">
                    <button type="button" className="btn btn-outline-secondary me-2" onClick={(e) => HandleBuyTicket(element,e)}>13.00</button>
                    <button type="button" className="btn btn-outline-secondary me-2" onClick={(e) => HandleBuyTicket(element,e)}>16.00</button>
                    <button type="button" className="btn btn-outline-secondary me-2" onClick={(e) => HandleBuyTicket(element,e)}>20.00</button>
                </Tab>
            ))}
        </Tabs>
        </div>
